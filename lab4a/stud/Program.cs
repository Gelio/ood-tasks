...
namespace Robotics
{
    class MainClass
    {
        public static void Main(string[] args)
        {
            ...
            {
                foreach (Part part in partsForServices)
                {
                    if (part.CanBeProducedByRobot(robot))
                        Console.WriteLine($"{robot.GetName()} tworzy {part.GetDescription()}");
                }
            }
            ...
            foreach (Robot parent in new Robot[] { new ABB(), new Kuka(), new Puma() })
                ...
                {
                    if (child.CanBeProducedByRobot(parent))
                        Console.WriteLine($"{parent.GetName()} tworzy {child.GetDescription()}");
                }
        }
    }
}
